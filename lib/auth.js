...
export function authenticate(req, res) {
  ...
  if (!accessToken && req.headers.authorization?.startsWith("Bearer ")) {
    accessToken = req.headers.authorization.split(" ")[1];  // Fixed
  }
  ...
  const newAccessToken = generateAccessToken({
    ...
    role: decodedRefresh.role || decodedRefresh.role_id,  // Handle both
  });
  ...
  if (needNewRefreshToken) {
    newRefreshToken = generateRefreshToken({
      user_id: decodedRefresh.user_id,
      role: decodedRefresh.role || decodedRefresh.role_id,
    });
  }
  ...
  res.setHeader("Set-Cookie", [
    cookie.serialize("accessToken", newAccessToken, {
      httpOnly: true,
      secure: isProd,
      sameSite: isProd ? "none" : "lax",  // Consider "lax" unless cross-site needed
      path: "/",
      maxAge: 15 * 60,
    }),
